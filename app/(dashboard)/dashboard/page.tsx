...
export default async function DashboardPage() {
  ...
  const hasPublishedProfile = !!(profileExists && profile.profile?.isPublished && user?.username && (profile.profile?.links?.length ?? 0) > 0);

  const shouldShowPublishAlert = !profileExists || !hasPublishedProfile;
  const canPublish = !!(profileExists && user?.username && (profile.profile?.links?.length ?? 0) > 0);
